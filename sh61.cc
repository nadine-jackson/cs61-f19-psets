...
pid_t command::make_child(pid_t pgid) {
    ...
    if(!f) //child will become program
      {
	printf("hi\n");
	char* strptr[args.size() + 1];
	int i = 0;
	for(; i < (signed)args.size(); i++)
	  {
	    strcpy(strptr[i], (char *) this->args[i].c_str());
	  }
	strptr[args.size()] = NULL;
	//printf("%s\n", strptr[0]);
	execvp(strptr[0], strptr);
      }
...
void run(command* c) {
  int status;
  waitpid(c->make_child(0), &status, 0);
